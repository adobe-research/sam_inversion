...
if __name__=="__main__":
    ...
    for im_path in l_im_paths:
        ...
        if True or not os.path.exists(os.path.join(args.output_path, bname, "final", f"inverted_latent.pt")):
            ...
            d_latents = {k:d_latents_init[k].detach().clone()*0.0 for k in d_latents_init}
            for k in d_latents:
            ...
            outf = os.path.join(args.output_path, bname, "final", f"reconstruction.png")
            if args.image_category=="cars":
                rec_full = rec_full[:,:,64:-64,:]
            tensor2pil(rec_full.squeeze(0)).save(outf)
            outf = os.path.join(args.output_path, bname, "final", f"inverted_latent.pt")
            ...
            outf = os.path.join(args.output_path, bname, "final", f"input.png")
            tensor2pil(img_full_t.squeeze(0)).save(outf)
        else:
            d_latents_out = torch.load(os.path.join(args.output_path, bname, "final", f"inverted_latent.pt"))
